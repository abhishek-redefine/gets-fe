...
const DriverProfile = () => {
  ...
  const initializer = async (resetFlag, filter = {}) => {
    try {
      ...
      const response = !resetFlag
        ? await ComplianceService.getAllDrivers(
            params.toString(),
            allValuesSearch
          )
        : await ComplianceService.getAllDrivers(params.toString, {});
      const { data } = response || {};
  ...
  const enableDisableDriver = async (id, isEnable, remark) => {
    try {
      setLoading(true);
      const response = await ComplianceService.enableDisableDriver(
        id,
        isEnable,
        remark
      );
  ...
  const uploadFunction = async (item) => {
    try {
      ...
      form.append(
        "model",
        '{"importJobDTO": {"importType": "IMPORT_TYPE_DRIVER","entityName": "DRIVER"}}'
      );
      ...
      if (response?.data?.isSuccessFul) {
        dispatch(
          toggleToast({
            message: "All Driver records uploaded successfully!",
            type: "success",
          })
        );
      } else {
        ...
        if (response?.data?.successRecords > 0) {
          ...
          dispatch(
            toggleToast({
              message: `Driver records failed to upload. Please try again later.`,
              type: "error",
            })
          );
        }
  ...
  return (
    <div>
      <div className="internalSettingContainer">
        {!addDriverOpen && !viewDriverOpen && !viewEhsDriverOpen && (
          <div>
            <div style={{ display: "flex", justifyContent: "start" }}>
              ...
              <div style={{ minWidth: "180px" }} className="form-control-input">
                <FormControl fullWidth>
                  ...
                  >
                    {!!driverStateList?.length &&
                      driverStateList.map((status, idx) => (
                        <MenuItem key={idx} value={status.value}>
                          {status.displayName}
                        </MenuItem>
                      ))}
        ...
        {addDriverOpen && (
          <AddNewDriver
            SetAddDriverOpen={cancelHandler}

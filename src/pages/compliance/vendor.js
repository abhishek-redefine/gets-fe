...
const Vendor = () => {
  ...
  const enableDisableVendor = async (id, isEnable, remark) => {
    try {
      setLoading(true);
      const response = await ComplianceService.enableDisableVendorCompany(
        id,
        isEnable,
        remark
      );
      if (response.status === 200) {
        dispatch(
          toggleToast({
            message: "Vendor company deactivated successfully!",
            type: "success",
          })
        );
        setOpen(false);
        ...
        dispatch(
          toggleToast({
            message:
              "Vendor company deactivation unsuccessful. Please try again later!",
            type: "error",
          })
        );
      }

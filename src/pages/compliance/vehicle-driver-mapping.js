...
const VehicleDriverMapping = () => {
  ...
  const driverMapping = async () => {
    ...
    try {
      setLoading(true);
      const response = await ComplianceService.forceMappingVehicle(
        mappedDriver,
        { vehicleDriverMappingDTO: selectedVehicle }
      );
      //const response = await ComplianceService.updateVehicle({ "vehicleDTO": selectedVehicle });
      if (response.status === 201) {
        dispatch(
          toggleToast({
            message: "Vehicle Driver mapping successful!",
            type: "success",
          })
        );
        setOpenModal(close);
        ...
        dispatch(
          toggleToast({
            message:
              "Vehicle Driver mapping unsuccessful.Please try again after some time!",
            type: "error",
          })
        );
      }
  ...
  const initializer = async () => {
    try {
      ...
      const response = await ComplianceService.getAllVehiclesMapping(
        params.toString(),
        {}
      );
    ...
    <div>
      <div className="internalSettingContainer">
        ...
        >
          <Box sx={style}>
            <div>
              <h4 style={{ paddingLeft: 15 }}>Are you sure ?</h4>
            </div>
            <div
              style={{
                display: "flex",

      ...
      <div className="mainSettingsContainer">
        ...
        <div className="currentStateContainer">
          {!!UserType?.length &&
            UserType.map(
              (userType, idx) =>
                userType.value !== SHIFT_TYPES.DRIVER && (
                  <button
                    key={idx}
        ...
        <div>
          {currentState === SHIFT_TYPES.CREATE_SHIFT_TIME && (
            <CreateShiftTime onSuccess={onSuccess} />
          )}
          {currentState === SHIFT_TYPES.SHIFT_TEAM_MAPPING && (
            <ShiftTeamMapping onSuccess={onSuccess} />
          )}
          {currentState === SHIFT_TYPES.SHIFT_CUTOFF && (
            <ShiftCutoff onSuccess={onSuccess} roleType={selectedRoleType} />
          )}
          {currentState === SHIFT_TYPES.VIEW_SHIFT_TIME && (
            <ViewShiftTime onSuccess={onSuccess} roleType={selectedRoleType} />
          )}
        </div>
      </div>
      {loading ? (

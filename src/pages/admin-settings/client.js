...
import { DATE_FORMAT, DATE_FORMAT_API, DEFAULT_PAGE_SIZE } from '@/constants/app.constants.';
import { Dialog, DialogActions, DialogContent, DialogTitle, FormControl, FormHelperText, InputLabel, MenuItem, Select, TextField } from '@mui/material';
...
const Client = () => {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values, { resetForm }) => {
            ...
            if (allValues.holidayDate) {
                allValues.holidayDate = moment(allValues.holidayDate).format(DATE_FORMAT_API);
            }
            try {
                await OfficeService.addHoliday({holiday: allValues});
                dispatch(toggleToast({ message: 'Holiday added successfully!', type: 'success' }));
                resetForm();
                setIsDialogOpen(false);
                ...
                console.error(e);
                dispatch(toggleToast({ message: e?.response?.data?.message || 'Error adding holiday, please try again later!', type: 'error' }));
            }
        }
    });
    ...
    const handleRemoveHoliday = async () => {
        try {
            ...
            dispatch(toggleToast({ message: 'Holiday removed successfully!', type: 'success' }));
            if (pagination.pageNo === 0) {
                fetchAllHolidays();
            } else {
                handlePageChange(0);
            ...
            console.error(e);
            dispatch(toggleToast({ message: e?.response?.data?.message || 'Error removing holiday, please try again later!', type: 'error' }));
        }
    };
        ...
        <div className='mainSettingsContainer'>
            ...
            <div className='internalSettingContainer'>
                <div className='btnContainer'>
                    <button onClick={() => setIsDialogOpen(true)} className='btn btn-primary'>Add Holiday</button>
                </div>
                <div className='gridContainer'>
                    <Grid onMenuItemClick={onMenuItemClick} handlePageChange={handlePageChange} pagination={paginationData} headers={headers} listing={holidaysListing} />
                </div>
            </div>
                ...
                <DialogContent>
                    <div>
                        ...
                        <div className='form-control-input'>
                            ...
                            helperText={touched.occasion && errors.occasion} onChange={handleChange}
                            required id="occasion" name="occasion" value={values.occasion} label="Occasion" variant="outlined" />
                        </div>
                        <div className='form-control-input'>
                            <InputLabel htmlFor="start-date" style={{marginBottom: "10px"}}>Date</InputLabel>
                            <LocalizationProvider dateAdapter={AdapterMoment}>
                                <DatePicker format={DATE_FORMAT} name="holidayDate" value={values.holidayDate ? moment(values.holidayDate) : null} onChange={(e) => handleDateChange(e, "holidayDate")} />
            ...
            <Dialog open={isRemoveDialogOpen} onClose={handleRemoveDialogClose}>
                ...
                <DialogContent>
                    <div>
                        <p>Do you really want to remove {holidayToRemove.occasion} - {holidayToRemove.holidayDate} holiday?</p>
                    </div>
                </DialogContent>
                <DialogActions>
                    <div className='popupBtnContainer'>
                        <button className='btn btn-secondary' onClick={handleRemoveDialogClose}>Cancel</button>
                        <button className='btn btn-primary' onClick={handleRemoveHoliday}>Yes</button>
                    </div>
                </DialogActions>
            </Dialog>
        </div>

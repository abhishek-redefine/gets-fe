...
const CreateBooking = () => {
  ...
  useEffect(() => {
    ...
    if (editBookingdata?.id) {
      ...
      setBookingFor(newEditInfo["bookingFor"]);
      ...
      if (newEditInfo.officeId && newEditInfo.transportType) {
        if (bookingFor === 3) {
      ...
      if (
        newEditInfo.transportType !== TRANSPORT_TYPES.CAB &&
        !nodalPoints?.length &&
        newEditInfo.officeId
      ) {
        fetchNodalPoints();
      }
    }
  ...
  const formik = useFormik({
    ...
    validationSchema: () => {
      ...
      if (
        !editFlag &&
        customiseSchRef.current.getAttribute("value") === "true"
      ) {
        console.log("customize");
        reValidationSchema = reValidationSchema.omit([
          "officeId",
          "logoutShift",
          "loginShift",
          "pickUpPoint",
    ...
    onSubmit: async (values) => {
      ...
      if (allValues.bookingFromDate) {
        allValues.bookingFromDate = moment(allValues.bookingFromDate).format(
          "YYYY-MM-DD"
        );
      }
      if (allValues.bookingToDate) {
        allValues.bookingToDate = moment(allValues.bookingToDate).format(
          "YYYY-MM-DD"
        );
      }
      if (bookingFor !== 1) {
  ...
  const fetchInOutTimes = async (isIn = false) => {
    try {
      setLoading(true);
      const startDate = moment(formik.values.bookingFromDate).format(
        DATE_FORMAT
      );
      const endDate = moment(formik.values.bookingToDate).format(DATE_FORMAT);
      const response = await BookingService.getLoginLogoutTimeWithDate(
        values.officeId,
        isIn,
  ...
  const fetchInOutTeamsTime = async (isIn = false) => {
    try {
      ...
      const endDate = moment(formik.values.bookingToDate).format(DATE_FORMAT);
      const response = await BookingService.getTeamLoginLogoutTimes(
        values.officeId,
        isIn,
        values.transportType,
        selectedTeamDetail.id,
        startDate,
        endDate
      );
      const { data } = response || {};
  ...
  const handleChangeMultiple = (e, idx, currentDate) => {
    ...
    if (currentCustomizedSch[idx]) {
      ...
      currentCustomizedSch[idx].scheduleDate =
        ...
        <div key={`multiDiv${currentIdx}`} className={styles.perDateContainer}>
          {date && (
            <>
              ...
              <span className={styles.dayAndDate}>
          ...
          <div className="form-control-input">
            <FormControl required fullWidth>
              ...
              <Select
                ...
                error={
                  isSubmit && !customizedScheduledBean?.[currentIdx]?.officeId
                }
                name="officeId"
                label="Primary Office"
                onChange={(e) =>
                  handleChangeMultiple(e, currentIdx, bookingDate)
                }
              >
                {!!offices?.length &&
          ...
          <div className="form-control-input">
            <FormControl required fullWidth>
              ...
              <Select
                ...
                error={
                  isSubmit &&
                  !customizedScheduledBean?.[currentIdx]?.bookingType
                }
              ...
              <div className="form-control-input">
                <FormControl required fullWidth>
                  ...
                  <Select
                    ...
                    id="loginShift"
                    value={
                      customizedScheduledBean?.[currentIdx]?.loginShift || ""
                    }
                    error={
                      isSubmit &&
                      !customizedScheduledBean?.[currentIdx]?.loginShift
                  ...
                  >
                    {!!multiLoginTimes[currentIdx]?.length &&
                      multiLoginTimes[currentIdx].map((time, idx) => (
                        <MenuItem key={idx} value={time.shiftTime}>
                          {time.shiftTime}
                        </MenuItem>
                      ))}
                  </Select>
              ...
              <div className="form-control-input">
                <FormControl required fullWidth>
                  ...
                  <Select
                    ...
                    id="logoutShift"
                    value={
                      customizedScheduledBean?.[currentIdx]?.logoutShift || ""
                    }
                    error={
                      isSubmit &&
                      !customizedScheduledBean?.[currentIdx]?.logoutShift
                  ...
                  >
                    {!!multiLogoutTimes[currentIdx]?.length &&
                      multiLogoutTimes[currentIdx].map((time, idx) => (
                        <MenuItem key={idx} value={time.shiftTime}>
                          {time.shiftTime}
                        </MenuItem>
                      ))}
              ...
              <div className="form-control-input">
                <FormControl required>
                  <FormGroup
                    ...
                    value={
                      customizedScheduledBean?.[currentIdx]
                        ?.nextDayLogOutCustomize || false
                    }
                    style={{ flexDirection: "row" }}
                  >
                    <FormControlLabel
                      name="nextDayLogOutCustomize"
                      control={
                        <Checkbox
                          value={
                            customizedScheduledBean?.[currentIdx]
              ...
              <div className="form-control-input">
                <FormControl required>
                  ...
                  <Select
                    ...
                    id="pickUpPoint"
                    value={
                      customizedScheduledBean?.[currentIdx]?.pickUpPoint || ""
                    }
                    error={
                      isSubmit &&
                      !customizedScheduledBean?.[currentIdx]?.pickUpPoint
                  ...
                  >
                    {!!nodalPoints?.length &&
                      nodalPoints.map((point, idx) => (
                        <MenuItem key={idx} value={point.locationName}>
                          {point.locationName}
                        </MenuItem>
                      ))}
                  </Select>
              ...
              <div className="form-control-input">
                <FormControl required>
                  ...
                  <Select
                    ...
                    id="dropPoint"
                    value={
                      customizedScheduledBean?.[currentIdx]?.dropPoint || ""
                    }
                    error={
                      isSubmit &&
                      !customizedScheduledBean?.[currentIdx]?.dropPoint
              ...
              {shiftType === "LOGIN" ? (
                <div className="form-control-input">
                  <FormControl required fullWidth>
                    ...
                    <Select
                      disabled={
                        values.bookingFromDate != "" &&
                        values.bookingToDate != ""
                          ? false
                          : true
                      }
                      labelId="login-label"
                    ...
                    {touched.loginShift && errors.loginShift && (
                      <FormHelperText className="errorHelperText">
                        {errors.loginShift}
                      </FormHelperText>
                    )}
                  ...
                  <div className="form-control-input">
                    <FormControl required fullWidth>
                      ...
                      <Select
                        ...
                        error={
                          touched.logoutShift && Boolean(errors.logoutShift)
                        }
                        name="logoutShift"
                        label="Logout Time"
                        onChange={(e) => {
                      ...
                      {touched.logoutShift && errors.logoutShift && (
                        <FormHelperText className="errorHelperText">
                          {errors.logoutShift}
                        </FormHelperText>
                      )}
                  ...
                  <div className="form-control-input">
                    <FormControl required>
                      <FormGroup
                        ...
                        error={
                          touched.nextDayLogOut && Boolean(errors.nextDayLogOut)
                        }
                        style={{ flexDirection: "row" }}
                      >
                        <FormControlLabel
                      ...
                      {touched.nextDayLogOut && errors.nextDayLogOut && (
                        <FormHelperText className="errorHelperText">
                          {errors.nextDayLogOut}
                        </FormHelperText>
                      )}
                    </FormControl>
                  </div>
  ...
  const renderDateRange = () => {
    const startDate = formik.values.bookingFromDate
      ? moment(formik.values.bookingFromDate, DATE_FORMAT)
      : "";
    const endDate = formik.values.bookingToDate
      ? moment(formik.values.bookingToDate, DATE_FORMAT)
      : "";
    ...
    if (
      ...
      while (currentDate.isSameOrBefore(endDate, "day")) {
        ...
        if (Object.keys(customisedData[count])?.length) {
          customisedData[count].scheduleDate =
            moment(currentDate).format("YYYY-MM-DD");
        }
  ...
  const fetchEmployeesByTeamId = async () => {
    try {
      ...
      const response = await BookingService.getEmployeesByTeamId(
        params.toString()
      );
      const { data } = response || {};
      const { paginatedResponse } = data || {};
  ...
  const handleMultiSubmit = () => {
    ...
    let isInvalid = customizedScheduledBean.some((individualBean) => {
      if (individualBean.officeId) {
        if (
          (individualBean.bookingType === "LOGIN" &&
            individualBean.loginShift) ||
          (individualBean.bookingType === "LOGOUT" &&
            individualBean.logoutShift) ||
          (individualBean.bookingType === "Both" &&
            individualBean.logoutShift &&
            individualBean.loginShift)
        ) {
          console.log(false);
    ...
    if (!isInvalid) {
      ...
      customizedScheduledBean.map((val, index) => {
        val.bookingType === "LOGOUT" &&
          val["loginShift"] &&
          delete val["loginShift"];
        val.bookingType === "LOGIN" &&
          val["logoutShift"] &&
          delete val["logoutShift"];
  ...
  const handleScheduleChange = (event) => {
    formik.setFieldValue(
      "isCustomiseSchedule",
      !Boolean(values.isCustomiseSchedule)
    );
  };
        ...
        <div className={styles.mainBookingContainer}>
                  ...
                  <div className={styles.bookingDateRangeInnerContainer}>
                    <LocalizationProvider dateAdapter={AdapterMoment}>
                      <DatePicker
                        ...
                        format={"DD-MM-YYYY"}
                        value={
                          values.bookingDate ? moment(values.bookingDate) : null
                        }
                      />
                  ...
                  <div className={styles.dateRangeInnerContainer}>
                    <LocalizationProvider dateAdapter={AdapterMoment}>
                      <FormControl>
                        <DatePicker
                          ...
                          value={
                            values.bookingFromDate
                              ? moment(values.bookingFromDate)
                              : null
                          }
                          onChange={(e) =>
                            handleDateChange(e, "bookingFromDate")
                          }
                        />
                        {touched.bookingFromDate && errors.bookingFromDate && (
                          <FormHelperText className="errorHelperText">
                      ...
                      <FormControl>
                        <DatePicker
                          ...
                          value={
                            values.bookingToDate
                              ? moment(values.bookingToDate)
                              : null
                          }
                          onChange={(e) => handleDateChange(e, "bookingToDate")}
                        />
                        {touched.bookingToDate && errors.bookingToDate && (
                          <FormHelperText className="errorHelperText">
                    ...
                    >
                      <FormControl required>
                        ...
                        >
                          <FormControlLabel
                            name="bookingOnWorkingDay"
                            value={true}
                            control={<Checkbox />}
                            label="Booking on week offs"
                          />
                        </FormGroup>
                      </FormControl>
                    </div>
          ...
          <div className={`${styles.mainBookingDataContainer} pt-2`}>
            <div>
              <div className="form-control-input">
                <FormControl required>
                  ...
                  <RadioGroup
                    ...
                    error={
                      touched.transportType && Boolean(errors.transportType)
                    }
                    name="transportType"
                  >
              ...
              <div className="form-control-input">
                {!editFlag && (
                  <FormGroup
                    ref={customiseSchRef}
                    value={values.isCustomiseSchedule}
                    onChange={handleScheduleChange}
                  >
                    <InputLabel>Customize Schedule</InputLabel>
                    <FormControlLabel
                      disabled={isSelectedDate()}
                      name="isCustomiseSchedule"
                      control={<Switch checked={values.isCustomiseSchedule} />}
                      label={values.isCustomiseSchedule ? "On" : "Off"}
                    />
                  </FormGroup>
                )}
              </div>
            </div>
            <div>{renderDateRange()}</div>
            {values.transportType !== TRANSPORT_TYPES.CAB &&
              !values.isCustomiseSchedule && (
                <div className={styles.pickAndDropContainer}>
                  <div className="form-control-input">
                    <FormControl required>
                      <InputLabel id="pickUpPoint-label">
                        Pickup Point
                      </InputLabel>
                      <Select
                        labelId="pickUpPoint-label"
                        ...
                        value={values.pickUpPoint}
                        error={
                          touched.pickUpPoint && Boolean(errors.pickUpPoint)
                        }
                        name="pickUpPoint"
                        label="Pickup Point"
                        onChange={handleChange}
                      ...
                      {touched.pickUpPoint && errors.pickUpPoint && (
                        <FormHelperText className="errorHelperText">
                          {errors.pickUpPoint}
                        </FormHelperText>
                      )}
                  ...
                  <div className="form-control-input">
                    <FormControl required>
                      ...
                      {touched.dropPoint && errors.dropPoint && (
                        <FormHelperText className="errorHelperText">
                          {errors.dropPoint}
                        </FormHelperText>
                      )}
            ...
            <div
              ...
              style={{ paddingBottom: "20px", flexDirection: "row-reverse" }}
            >
              <div>
                {editFlag && (
                  <button onClick={backHandler} className="btn btn-secondary">
                    Back
                  </button>
                )}
                ...
                >
                  {editFlag ? "Update" : "Create"} Booking
                </button>
              </div>
            </div>
          </div>
      ...
      {currentBookingFlow === 2 && (
        <div>
          <div className={styles.mainBookingContainer}>
            ...
            {!!(selectedUsers?.length && selectedOtherUserDetails) && (
              <div className={styles.mainBookingDataContainerUser}>
                ...
                <div className={styles.userDetailsContainer}>
                  ...
                  <p style={{ textTransform: "capitalize" }}>
                    {selectedOtherUserDetails.gender.toLowerCase()}
                  </p>
                </div>
                <div className={styles.userDetailsContainer}>
                ...
                <div className={styles.userDetailsContainer}>
                  <button
                    onClick={() => setIsViewMore((previous) => !previous)}
        ...
        <div>
          <div className={styles.mainBookingContainer}>
            ...
            <div className={styles.mainBookingDataContainer}>
              {teamFlow === 1 && (
                <Grid
                  onRadioClick={onTeamRadioClick}
                  handlePageChange={handleTeamPageChange}
                  headers={teamHeaders}
                  listing={teamListing}
                  pagination={teamPaginationData}
                />
              )}
              {teamFlow === 2 && (
                <Grid
                  onCheckboxClick={onTeamCheckboxClick}
                  headers={teamUserHeaders}
                  listing={teamUserListing}
                />
              )}
            </div>
            <div
            ...
            >
              <div>
                <button onClick={handleTeamNext} className="btn btn-primary">
                  Next
                </button>
              </div>
            </div>

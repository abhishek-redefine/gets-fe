...
const AdminManagement = ({ roleType, onSuccess }) => {
  ...
  const uploadFunction = async (item) => {
    try {
      ...
      form.append(
        "model",
        '{"importJobDTO": {"importType": "IMPORT_TYPE_ADMIN_USER","entityName": "ADMIN USER"}}'
      );
      ...
      if (response?.data?.isSuccessFul) {
        dispatch(
          toggleToast({
            message: "All Admin records uploaded successfully!",
            type: "success",
          })
        );
      } else {
        ...
        if (response?.data?.successRecords > 0) {
          dispatch(
            toggleToast({
              message: `${response?.data?.successRecords} out of ${response?.data?.totalRecords} Admin records uploaded successfully!`,
              type: "success",
            })
          );
        } else {
          dispatch(
            toggleToast({
              message: `Admin records failed to upload. Please try again later.`,
              type: "error",
            })
          );
        }

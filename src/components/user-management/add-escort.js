...
const AddEscort = ({ onUserSuccess, editEmployeeData }) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        setLoading(true);
        if (initialValues?.id) {
          await OfficeService.updateEscort({
            escort: { ...initialValues, ...allValues },
          });
          dispatch(
            toggleToast({
        ...
        dispatch(
          toggleToast({
            message:
              ...
              "Error adding escort, please try again later!",
            type: "error",
          })
        );
      } finally {
  ...
  return (
    <div>
      <h4 className="pageSubHeading">
        {editEmployeeData?.id ? "Edit" : "Add"} Escort
      </h4>
        ...
        <div>
          <div className="form-control-input">
            <TextField
              ...
              helperText={touched.escortId && errors.empId}
              onChange={handleChange}
              required
              id="escortId"
              name="escortId"
              value={values.escortId}
              label="Escort ID"
              variant="outlined"
            />
          </div>
        ...
        <div className="addBtnContainer">
          ...
          <div>
            ...
            <button onClick={handleSubmit} className="btn btn-primary">
              {editEmployeeData?.id ? "Update" : "Create"} Escort
            </button>
          </div>
        </div>
      </div>

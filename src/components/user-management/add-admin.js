...
const AddAdmin = ({ roleType, onUserSuccess, editEmployeeData }) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        setLoading(true);
        if (initialValues?.id) {
          await OfficeService.updateAdmin({
            adminUser: { ...initialValues, ...allValues },
          });
          dispatch(
            toggleToast({
        ...
        dispatch(
          toggleToast({
            message:
              ...
              "Error adding admin, please try again later!",
            type: "error",
          })
        );
      } finally {
  ...
  return (
    <div>
      <h4 className="pageSubHeading">
        {editEmployeeData?.id ? "Edit" : "Add"} Admin
      </h4>
      <div className="addUpdateFormContainer">
        ...
        <div className="addBtnContainer">
          ...
          <div>
            ...
            >
              {editEmployeeData?.id ? "Update" : "Create"} Admin
            </button>
          </div>
        </div>
      </div>

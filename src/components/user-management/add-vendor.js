...
const AddVendor = ({ roleType, onUserSuccess, editEmployeeData }) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        setLoading(true);
        if (initialValues?.id) {
          await OfficeService.updateVendor({
            vendorTeamDTO: { ...initialValues, ...allValues },
          });
          dispatch(
            toggleToast({
        ...
        dispatch(
          toggleToast({
            message:
              ...
              "Error adding vendor, please try again later!",
            type: "error",
          })
        );
      } finally {
  ...
  return (
    <div>
      <h4 className="pageSubHeading">
        {editEmployeeData?.id ? "Edit" : "Add"} Vendor
      </h4>
      <div className="addUpdateFormContainer">
        ...
        <div>
          ...
          <div className="form-control-input">
            <TextField
              value={values.contactPersonMob}
              onChange={handleChange}
              error={
                touched.contactPersonMob && Boolean(errors.contactPersonMob)
              }
              helperText={touched.contactPersonMob && errors.contactPersonMob}
              name="contactPersonMob"
              id="contactPersonMob"
        ...
        <div className="addBtnContainer">
          ...
          <div>
            ...
            >
              {editEmployeeData?.id ? "Update" : "Create"} Vendor
            </button>
          </div>
        </div>
      </div>

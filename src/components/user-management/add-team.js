...
const AddTeam = ({ onUserSuccess, editEmployeeData }) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        setLoading(true);
        if (initialValues?.id) {
          await OfficeService.updateTeams({
            team: { ...initialValues, ...allValues },
          });
          dispatch(
            toggleToast({
        ...
        dispatch(
          toggleToast({
            message:
              ...
              "Error adding team, please try again later!",
            type: "error",
          })
        );
      } finally {
  ...
  return (
    <div>
      <h4 className="pageSubHeading">
        {editEmployeeData?.id ? "Edit" : "Add"} Team
      </h4>
      <div className="addUpdateFormContainer">
                ...
                {!!offices?.length &&
                  offices.map((office, idx) => (
                    <MenuItem key={idx} value={office.officeId}>
                      <Checkbox
                        checked={values.officeIds.indexOf(office.officeId) > -1}
                      />
                      <ListItemText
                        primary={`${getFormattedLabel(office.officeId)}, ${
                          office.address
                        }`}
                      />
                    </MenuItem>
          ...
          <div className="form-control-input">
            <FormControl variant="outlined">
              <Autocomplete
                ...
                onChange={(e, val) =>
                  onChangeHandler(val, "managerIds", "empId")
                }
                ...
                renderInput={(params) => (
                  <TextField
                    {...params}
                    label="Search Team Manager"
                    onChange={searchForRM}
                  />
                )}
              />
              {touched.managerIds && errors.managerIds && (
                <FormHelperText className="errorHelperText">
                  {errors.managerIds}
                </FormHelperText>
              )}
        ...
        <div>
          ...
          <div className="form-control-input">
            <TextField
              error={
                touched.sendNotification && Boolean(errors.sendNotification)
              }
              onChange={handleChange}
              helperText={touched.sendNotification && errors.sendNotification}
              required
              id="sendNotification"

...
const validationSchemaStepOne = object({
  ...
  pollutionExpiryDate: string().required(
    "Pollution Certificate Expiry Date is required"
  ),
  statePermitExpiryDate: string().required(
    "State Permit Expiry Date is required"
  ),
  nationalPermitExpiryDate: string().required(
    "National Permit Expiry Date is required"
  ),
...
const validationSchemaStepTwo = object({
  registrationCertificateUrl: string().required(
...
const validationSchemaStepFour = object({
  pollutionCertificateUrl: string().required(
...
const validationSchemaStepEight = object({
  nationalPermitUrl: string().required(
    "National Permit Certificate is required"
  ),
});
...
const AddNewVehicle = ({ EditVehicleData, SetAddVehicleOpen }) => {
  ...
  const onChangeHandlerDropdown = (newValue) => {
    ...
    else if (name === "rfidStatus")
      allValues.rfidStatus = newValue.target.value;
      ...
      if (EditVehicleData) {
        ...
        if (response.status === 200) {
          ...
          dispatch(
            toggleToast({
              message:
                "Vehicle details not updated. Please try again after some time.",
              type: "error",
            })
          );
          return false;
        ...
        setLoading(true);
        // await new Promise((resolve) => setTimeout(resolve, 5000));
        const response = await ComplianceService.createVehicle({
          vehicleDTO: apiData,
        });
        console.log(response);
        if (response.status === 201) {
          ...
          dispatch(
            toggleToast({
              message:
                "Vehicle details not added. Please try again after some time.",
              type: "error",
            })
          );
          return false;
  ...
  const uploadDocumentFormSubmit = async (id, role, documentToUpload, data) => {
    try {
      ...
      if (response.status === 200) {
        dispatch(
          toggleToast({
            message: `${documentToUpload
              .replace("/", "")
              .replace("_", " ")} uploaded successfully!`,
            type: "success",
          })
        );
        if (documentToUpload === "/REGISTRATION_CERTIFICATE") {
          EditVehicleData.registrationCertificateUrl =
            response.data.vehicleDTO.registrationCertificateUrl;
  ...
  //Geocode Modal//
  const [openGeocodeModal, setGeocodeOpenModal] = useState(false);
  const handleGeocodeModalOpen = () => {
    console.log("inside geocode modal");
    setGeocodeOpenModal(true);
  };
  const handleGeocodeModalClose = () => {
    console.log("geocode modal closed");
    setGeocodeOpenModal(false);
  };
  ...
  return (
    <div>
      ...
      <MultiStepForm
        ...
        onSubmit={async () => {
          const response = await ComplianceService.changeStatusVehicle(
            vehicleId
          );
          if (response) {
      ...
      >
        ...
        >
          <div style={{ display: "flex", alignItems: "center" }}>
            ...
            />
            <TextInputField name="stickerNumber" label="Sticker Number" />
            {/* <TextInputField
                            name="vehicleType"
                            label="Vehicle Type" /> */}
            <SelectInputField
          ...
          <div style={{ display: "flex", alignItems: "center" }}>
            ...
            />
            <TextInputField name="modelYear" label="Model Year" />
            <TextInputField name="vehicleMake" label="Vehicle Make" />
            <TextInputField name="vehicleModel" label="Vehicle Model" />
          </div>
          <div style={{ display: "flex", alignItems: "center" }}>
            <SelectInputField
              name="fuelType"
          ...
          <div style={{ display: "flex", alignItems: "center" }}>
            <DateInputField
              ...
              minDate={dayjs()}
            />
            <TextInputField name="garageLocation" label="Garage Location" />
            <SelectInputField
              name="ehsStatus"
              label="EHS Status"
              genderList={ehsTypes}
          ...
          <div style={{ display: "flex", alignItems: "center" }}>
            ...
            <SelectInputField
              ...
              genderList={gpsTypes}
            />
            <TextInputField name="vehicleRemarks" label="Vehicle Remarks" />
          </div>
          {/* <div style={{ display: 'flex', alignItems: 'center' }}>
                        <div className='form-control-input'>
                            <FormControl variant="outlined">

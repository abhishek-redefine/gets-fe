...
export const validationSchema = yup.object({
  ...
  ehsFrequency: yup
    .string("Enter EHS Frequency")
    .required("Enter EHS Frequency"),
  ehsAppliedOn: yup
    .string("Enter EHS Applied On")
    .required("Enter EHS Applied On"),
});
...
const AddEHS = ({ EditEhsData, SetIsAddEhs }) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        setLoading(true);
        if (EditEhsData?.id) {
          values.ehsAppliedOn === "Driver"
            ? (values["ehsAppliedOnDriver"] = true)
            : (values["ehsAppliedOnDriver"] = false);
          values.officeIds = [values.officeIds];
          const response = await ComplianceService.updateEHS({ ehs: values });
          if (response.status === 200) {
            dispatch(
              toggleToast({
                message: "EHS details updated successfully!",
                type: "success",
              })
            );
            SetIsAddEhs(false);
            ...
            dispatch(
              toggleToast({
                message: "EHS details not updated. Please try again later!",
                type: "error",
              })
            );
            SetIsAddEhs(false);
          ...
          if (response.status === 201) {
            dispatch(
              toggleToast({
                message: "EHS details added successfully!",
                type: "success",
              })
            );
            SetIsAddEhs(false);
            ...
            dispatch(
              toggleToast({
                message: "EHS details not added. Please try again later!",
                type: "error",
              })
            );
            SetIsAddEhs(false);
  ...
  const fetchAllMasterData = async () => {
    try {
      setLoading(true);
      const EhsCategoryResponse = await ComplianceService.getMasterData(
        "EhsCategory"
      );
      const EhsFrequencyResponse = await ComplianceService.getMasterData(
        "EhsFrequency"
      );
      const EhsMandateResponse = await ComplianceService.getMasterData(
        "EhsMandate"
      );
      const TransportTypeResponse = await ComplianceService.getMasterData(
        "TransportType"
      );
      ...
      <div className="addUpdateFormContainer">
        <div>
          <div className="form-control-input">
            <FormControl required fullWidth>
              ...
              {touched.officeIds && errors.officeIds && (
          ...
          <div className="form-control-input">
            <FormControl required fullWidth>
              ...
              <Select
                ...
                onChange={handleChange}
              >
                {!!ehsCategoryList?.length &&
                  ehsCategoryList.map((category, idx) => (
                    <MenuItem key={idx} value={category.displayName}>
                      {category.displayName}
                    </MenuItem>
                  ))}
              </Select>
              {touched.ehsCategory && errors.ehsCategory && (
                <FormHelperText className="errorHelperText">
                  {errors.ehsCategory}
                </FormHelperText>
              )}
          ...
          <div className="form-control-input">
            <FormControl required fullWidth>
              ...
              >
                {!!ehsMandate?.length &&
                  ehsMandate.map((category, idx) => (
                    <MenuItem key={idx} value={category.displayName}>
                      {category.displayName}
                    </MenuItem>
                  ))}
              </Select>
              {touched.ehsMandate && errors.ehsMandate && (
                <FormHelperText className="errorHelperText">
                  {errors.ehsMandate}
                </FormHelperText>
              )}
          ...
          <div className="form-control-input">
            <FormControl required fullWidth>
              ...
              >
                {!!ehsFrequency?.length &&
                  ehsFrequency.map((category, idx) => (
                    <MenuItem key={idx} value={category.displayName}>
                      {category.displayName}
                    </MenuItem>
                  ))}
              </Select>
              {touched.ehsFrequency && errors.ehsFrequency && (
                <FormHelperText className="errorHelperText">
                  {errors.ehsFrequency}
                </FormHelperText>
              )}
          ...
          <div className="form-control-input">
            <TextField
              ...
              helperText={touched.ehsTitle && errors.ehsTitle}
              onChange={handleChange}
              required
              id="ehsTitle"
              name="ehsTitle"
              value={values.ehsTitle}
              label="EHS Title"
              variant="outlined"
            />
          </div>
        ...
        <div className="addBtnContainer">
          ...
          <div>
            ...
            >
              {EditEhsData?.id ? "Update" : "Create"} EHS
            </button>
          </div>
        </div>
      </div>

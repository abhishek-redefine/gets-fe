...
export const validationSchema = yup.object({
  ...
  penaltyCategory: yup
    .string("Enter Penalty Category")
    .required("Enter Penalty Category"),
  ...
  penaltyAmount: yup
    .string("Enter Penalty Amount")
    .required("Enter Penalty Amount"),
  penaltyType: yup.string("Enter Penalty Type").required("Enter Penalty Type"),
  penaltyActionRequired: yup
    .string("Enter Penalty Action Required")
    .required("Enter Penalty Action Required"),
});
    ...
    onSubmit: async (values) => {
      try {
        ...
        if (EditPenaltyData.id) {
          ...
          const response = await ComplianceService.updatePenalty({
            penalty: values,
          });
          console.log(response.status);
          isAddPenalty();
        } else {
          const response = await ComplianceService.createPenalty({
            penalty: values,
          });
          console.log(response.status);
          isAddPenalty();
  ...
  return (
    <div>
      <h4 className="pageSubHeading">
        {EditPenaltyData?.id ? "Edit" : "Add"} Penalty
      </h4>
      <div className="addUpdateFormContainer">
        <div>
          ...
          <div className="form-control-input">
            <TextField
              ...
              helperText={touched.penaltyCategory && errors.penaltyCategory}
              onChange={handleChange}
              required
              id="penaltyCategory"
              name="penaltyCategory"
              value={values.penaltyCategory}
          ...
          <div className="form-control-input">
            <TextField
              ...
              helperText={touched.slaCategory && errors.slaCategory}
              onChange={handleChange}
              required
              id="slaCategory"
              name="slaCategory"
              value={values.slaCategory}
              label="SLA Category"
              variant="outlined"
            />
          </div>
          <div className="form-control-input">
            <TextField
              ...
              helperText={touched.penaltyType && errors.penaltyType}
              onChange={handleChange}
              required
              id="penaltyType"
              name="penaltyType"
          ...
          <div className="form-control-input">
            <TextField
              ...
              helperText={touched.penaltyName && errors.penaltyName}
              onChange={handleChange}
              required
              id="penaltyName"
              name="penaltyName"
              value={values.penaltyName}
              label="Penalty Name"
              variant="outlined"
            />
          </div>
          ...
          <div className="form-control-input">
            <FormControl>
              ...
              <RadioGroup
                ...
                error={
                  touched.penaltyActionRequired &&
                  Boolean(errors.penaltyActionRequired)
                }
        ...
        <div className="addBtnContainer">
          ...
          <div>
            <button
              onClick={() => isAddPenalty()}
              className="btn btn-secondary"
            >
              Back
            </button>
            <button onClick={handleSubmit} className="btn btn-primary">
              {EditPenaltyData?.id ? "Update" : "Create"} Penalty
            </button>
          </div>
        </div>
      </div>
